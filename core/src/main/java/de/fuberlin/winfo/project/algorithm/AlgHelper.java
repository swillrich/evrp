...
public class AlgHelper {
	...
	public static ModRoute getRouteHelper(Algorithm alg, Vehicle vehicle, Node depot) {
		return new ModRoute(alg, vehicle, depot);
	}
	...
	public static class ModRoute implements Route {
		private Route route;
		private NetworkProvider networkProvider;
		private Node depot;

		public ModRoute(Algorithm algorithm, Vehicle vehicle, Node depot) {
		...
		/**
		 * Returns the remaining amount of vehicle cargo capacity after adding
		 * the given customer.
		 */
		 ...
		 * needed for driving back to the depot. If the given edge representing
		 * the depot back link, this is not taking into account.
		 */
		public boolean isSufficientEnergyAvailable(Order newOrder, int atEdgeWithIndex) throws Exception {
			...
			Edge edgeToNewNode = networkProvider.getEdges()[route.getWay().get(atEdgeWithIndex).getEdge().getStart()
					.getId()][node.getId()];
			// Second new edge
			Edge edgeFromNewNode = networkProvider.getEdges()[node.getId()][route.getWay().get(atEdgeWithIndex)
					.getEdge().getEnd().getId()];

			double need = route.getWay().get(atEdgeWithIndex).getCurrentVehicleCargoWeight();

			double capacityLeft = 0;
			try {
				capacityLeft = computeRemainingVehicleBatteryCapacity(atEdgeWithIndex, newOrder.getNeedAsWeight(),
						false);
			} catch (Exception e) {
				return false;
			}
			capacityLeft -= AlgHelper.computeEnergyConsumptionOfEdge(route.getVehicle(),
					need + newOrder.getNeedAsWeight(), edgeToNewNode.getDistance());
		...
		private int getTimeWindowStartFromEndNodeFromEdge(List<UsedEdge> way, int edgeIndex) {
			...
			if (way.get(edgeIndex) instanceof Delivery) {
				try {
					startTimeWindowAtEdgeEnd = ((Delivery) way.get(edgeIndex)).getOrder().getTimeWindow()
							.getStartInSec();
		...
		public boolean isSufficientTimeWindowAvailable(Order newOrder, int usedEdgeIndex) {
			...
			if (availableTime >= neededTime) {
				if (edgeStartInSec + timeStartNodeToNewNode - possibleShiftEdgeStart <= newOrder_TW_EndInSec
						&& edgeEndInSec - timeNewNodeToEndNode - serviceTimeAtNewNode
								+ possibleShiftEdgeEnd >= newOrder_TW_StartInSec) {
					return true;
				}
			}
			return false;
		 ...
		 * <br>
		 * Precondition for executing this method is pre-calculating all
		 * remaining cargo capacity values for each node.
		 * 
		 * @throws Exception
		 */
		...
		private void reinitializeRoute() throws Exception {
			for (int i = 0; i < route.getWay().size(); i++) {
				 ...
				 * considered. This procedure works inverse. Put another way,
				 * it's an iterative loop, which starts at end and terminates at
				 * start. Only this way it's possible to compute the current
				 * weight at each position correctly.
				 */
		...
		private void computeArrival(UsedEdge usedEdge) {
			...
			if (timeWindow != null) {
				...
				usedEdge.getDuration()
						.setEndInSec(usedEdge.getDuration().getStartInSec() + usedEdge.getEdge().getTime());
			}
		}
